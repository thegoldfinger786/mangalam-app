...
_Last updated: 2026-08-29 (batch 3 — honest progress/streak; on branch `ux/batch-3-honest-progress`)_
...
| P2 | 38 |
| KEEP | 11 |
| Implemented / Merged | 8 merged (Batch 1 · PR #1; Batch 2 · PR #2) + Batch 3 on branch (UX-03, HOME-09, STREAK-01, STREAK-02, STREAK-03, STREAK-04, STREAK-06) |
...
| UX-03 | Two streak widgets with different logic & week-start | NEEDS CHANGE | P1 | **MERGED** (Batch 3 · PR #4) | Home, Streaks — now one `WeeklyStreak` component + one data source |
...
| ID | Screen / Function | Finding | Class | Rec | Pri | Conf | Status | Notes |
|---|---|---|---|---|---|---|---|---|
| STREAK-01 | Fabricated "Total Time" | [SRC+LIVE] `streakCount × 10m`, shown "~10m", unrelated to actual listening | NEEDS CHANGE | Remove, or make it real (measured listening time) | P1 | High | **MERGED** (Batch 3 · PR #4) | ref UX-04. [SRC] Confirmed: no listening-duration data exists anywhere in the schema — `user_daily_usage` stores only `sessions_used` (a per-day session *count*). "Total Time" removed; the second Streaks stat card is now **"Sessions this week"** — a measured sum of `sessions_used` over the last 7 days. |
| STREAK-02 | "streak" not consecutive | [SRC+LIVE] Count of usage-day rows in last 30, labeled "N Day Journey" | NEEDS IMPROVEMENT | Relabel honestly ("days practised"), or compute a real streak | P1 | High | **MERGED** (Batch 3 · PR #4) | ref UX-04, STREAK-08. Tracking logic unchanged (not a bug). Hero relabelled "**N days of practice**" — an honest count of the listener's distinct active days. Note: the underlying query caps at the 30 most recent active days, so the number can plateau at 30 for a very consistent listener — recorded as STREAK-08. |
| STREAK-03 | Broken emoji glyph | [LIVE] 🔥 in "Day Journey 🔥" renders as a missing-glyph box on the test device | NEEDS IMPROVEMENT | Use an icon component, not a raw emoji | P2 | High | **MERGED** (Batch 3 · PR #4) | Resolved by removing the flame entirely (STREAK-06) — no raw emoji remains in the streak experience. |
| STREAK-04 | Duplicate week widget | [SRC+LIVE] Different from Home's; different week start ("F S S M T W T"), no dates | NEEDS CHANGE | One shared widget | P1 | High | **MERGED** (Batch 3 · PR #4) | ref UX-03. The inline 7-dot tracker in `StreaksScreen` was deleted; the screen now renders `<WeeklyStreak activeDates={…} />`, the same component Home uses. |
...
| 6 | Honest metrics + one streak widget | UX-03, UX-04, STREAK-01, STREAK-02, STREAK-04 | P1 | **MERGED** (Batch 3 · PR #4) — remaining: LIB-06 ("completed" on open), STREAK-08 (30-day cap) |
